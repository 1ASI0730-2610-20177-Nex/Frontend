  ...
  return consumptionsByDevice.value
      .map((group) => {
        ...
        return {
          ...
          deviceName: device ? device.name : t.value.deviceFallback(group.deviceId),
...
const navigateToConsumptions = () => router.push({ name: 'analytics-consumptions' });
</script>

<template>
  <section class="page">
    <header class="page-head">
      <h1 class="page-title">{{ t.byDevice }}</h1>
    ...
    <div v-else class="analytics-grid">
      <article v-for="summary in deviceSummaries" :key="summary.deviceId" class="analytics-card">
        <header class="analytics-card-head">
          <div class="analytics-card-info">
            <h2 class="analytics-card-title">{{ summary.deviceName }}</h2>
            <p class="analytics-card-meta">{{ summary.recordCount }} {{ t.records }}</p>
          </div>
...
.analytics-card-head {
  ...
  background: linear-gradient(135deg, var(--color-background-main) 0%, var(--color-brand-primary-soft) 100%);
  border-bottom: 1px solid var(--color-border-default);
}
.analytics-card-info { min-width: 0; flex: 1; }
...
.analytics-card-body { padding: 0.25rem 0; }
.analytics-table {
  width: 100%;
...
.analytics-table tbody tr:nth-child(even) { background: var(--table-row-stripe); }
.analytics-table tbody tr:last-child td { border-bottom: none; }
.analytics-table tbody tr:hover { background: var(--table-row-hover); }
.col-kwh {
  text-align: right;
  font-weight: var(--font-weight-semibold);
  color: var(--color-text-primary);
