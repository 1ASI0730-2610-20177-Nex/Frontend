...
<template>
  <section class="page">
    <header class="page-head">
      <h1 class="page-title">{{ isEdit ? 'Edit Consumption' : 'New Consumption' }}</h1>
    </header>
    ...
    <form class="form card" @submit.prevent="saveConsumption">
      <div class="form-field">
        ...
        <select id="device" v-model="form.deviceId" class="form-select" required>
          <option disabled :value="null">Select a device</option>
          <option v-for="device in managementStore.devices" :key="device.id" :value="device.id">
            {{ device.name }}
          </option>
        </select>
      </div>
